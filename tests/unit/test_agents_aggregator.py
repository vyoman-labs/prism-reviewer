...
class TestRenderMarkdown:
    ...
    def test_table_cell_sanitizes_newlines_and_pipes(self) -> None:
        """Table cells containing newlines, <br> tags, or pipes must be sanitized to stay on one row."""
        f = _make_finding(severity="MAJOR")
        f["message"] = "Multi-line\nmessage\nwith | pipe <br> tag"
        report = _render_markdown("PR", [f], {"CRITICAL": 0, "MAJOR": 1, "ADVISORY": 0})
        
        # Finding table row must be on a single physical line in markdown
        table_rows = [line for line in report.splitlines() if "| ⚠️ MAJOR" in line or "warden" in line]
        assert len(table_rows) == 1
        row = table_rows[0]
        assert "Multi-line message with \\| pipe tag" in row
        assert "\n" not in row

    def test_sanitize_table_cell_helper(self) -> None:
        """_sanitize_table_cell must strip newlines, escape pipes, and collapse whitespace."""
        from prism_reviewer.agents.aggregator import _sanitize_table_cell
        assert _sanitize_table_cell("") == ""
        assert _sanitize_table_cell("  hello   world  ") == "hello world"
        assert _sanitize_table_cell("line1\r\nline2\nline3") == "line1 line2 line3"
