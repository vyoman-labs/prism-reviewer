...
### Fixed
- **Markdown Table Cell Sanitization**: Added `_sanitize_table_cell` in `aggregator.py` to strip internal newlines (`\n`, `\r`), HTML break tags (`<br>`), and escape pipe characters (`|`) in finding messages and filenames, ensuring Markdown findings table rows stay on single physical lines and column formatting is not broken in PR summary comments.
